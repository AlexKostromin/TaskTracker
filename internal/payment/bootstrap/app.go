...
import (
	"context"
	"gitlab.com/godevs2/micro/internal/payment/config"
...
func New() *App {
	...
	storage := providePaymentStorage()
	paymentService := providePaymentService(storage)
	paymentHandler := providePaymentHandler(cfg.GRPCPort, paymentService)

	app := &App{
		grpcServer: paymentHandler,
		config:     cfg,
	}
